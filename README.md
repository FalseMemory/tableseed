...
| 命令 | 检查什么 |
| --- | --- |
| `pytest` | 297 条单元 / 集成测试（含 26 条边界攻击） |
| `python scripts/check_modules.py` | 用 Node 跑**真实前端代码**，逐个驱动 13 个功能模块，断言渲染产出 |
| `python scripts/check_modules.py --live` | 对**真实运行的服务**做 18 项接口冒烟（含 SQL 台只读防护） |
| `python scripts/verify_ui.py` | 用系统 Chrome 无头**真跑页面**，检查关键元素（前端启动类问题只能这样抓到） |

> 经验：接口全绿 ≠ 页面能用。前端改动务必跑后两条。

## 🔌 不连数据库也能用（离线场景）

生产库往往不允许直连（只走工单 / 第三方平台）—— tableseed 在这种情况下依然可用：

| 场景 | 做法 |
| --- | --- |
| **无连接生成 SQL** | 结果预览页点「**生成 INSERT 语句**」→ 选方言（MySQL / PostgreSQL / Oracle）→ 复制或下载 `.sql`，到对方平台执行 |
| 命令行出文件 | `tableseed gen -c x.yaml -o out/` 直接落盘 SQL / CSV |
| 只有表结构，连不上库 | 「生成配置」页**手贴** DDL + 几条 INSERT 样例即可生成配置草稿 |
| 想校验数据规则 | `tableseed verify -c x.yaml` 在内存里跑不变量，不需要数据库 |

生成的 SQL 带注释头（行数 / seed / 方言 / 执行顺序），多表**按依赖顺序**排列，
父表在前，可直接整体执行。
